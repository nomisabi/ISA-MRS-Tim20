...
public class RestoranApplicationTests {
	...
	@Autowired
	DrinkMenuItemReservationRepository drinkMenuItemReservationRepository;
	@Autowired
	...
	public void order() throws Exception {
		...
		tr.setReservation(r);
		tableReservationRepository.save(tr);
		Drink dr = new Drink("sok", "sok");
		drinkRepository.save(dr);
		DrinkMenuItem d = new DrinkMenuItem();
		d.setDrink(dr);
		dmenuItemRepository.save(d);
		d.setPrice(150);
		DrinkMenuItemReservation drink = new DrinkMenuItemReservation(d, guest, r);
		drinkMenuItemReservationRepository.save(drink);
		ArrayList<DrinkMenuItemReservation> drinkMenuItems = new ArrayList<>();
		drinkMenuItems.add(drink);
		ArrayList<MenuItemReservation> menuItems = new ArrayList<>();
		ItemsReservation item = new ItemsReservation(menuItems, drinkMenuItems);

		this.mvc.perform(post("/api/restaurant/order").contentType(MediaType.APPLICATION_JSON_VALUE)
				.content(IntegrationTestUtils.convertObjectToJsonBytes(item))).andExpect(status().isOk());

		Collection<DrinkMenuItemReservation> drinkIt = drinkMenuItemReservationRepository.getMenuItems(r.getId(),
				guest.getId());
		assertEquals(1, drinkIt.size());
	}
	...
	public void getRequests() throws Exception {
		...
		Guest friend = new Guest("nevena5695@gmail.com", "password12345", "name5", "name5");
		guestRepository.save(friend);
		GuestReservation gr = new GuestReservation(friend, r, false);
		guestReservationRepository.save(gr);
		VerificationToken vr = new VerificationToken(gr, UUID.randomUUID().toString());
		verificationRepository.save(vr);

		ConfirmInvite ci = new ConfirmInvite();
		ci.setToken(vr.getToken());

		this.mvc.perform(post("/api/reservation").contentType(MediaType.APPLICATION_JSON_VALUE)
				.content(IntegrationTestUtils.convertObjectToJsonBytes(ci))).andExpect(status().isOk());

	}

	@Test
	public void confirmInvite() throws Exception {
		Reservation r = new Reservation(restaurant, "20/06/2017 15:00", "20/06/2017 16:00");
		reservationReposotory.save(r);
		TableReservation tr = new TableReservation(table, r.getStartTime(), r.getEndTime());
		tr.setReservation(r);
		...
		guestRepository.save(friend);
		GuestReservation gr = new GuestReservation(friend, r, false);
		gr = guestReservationRepository.save(gr);
		VerificationToken vr = new VerificationToken(gr, UUID.randomUUID().toString());
		verificationRepository.save(vr);
		...
		this.mvc.perform(post("/api/reservation/confirm").contentType(MediaType.APPLICATION_JSON_VALUE)
				.content(IntegrationTestUtils.convertObjectToJsonBytes(ci))).andExpect(status().isOk());
	...
	public void deleteInvite() throws Exception {
		...
		this.mvc.perform(post("/api/reservation/delete").contentType(MediaType.APPLICATION_JSON_VALUE)
				.content(IntegrationTestUtils.convertObjectToJsonBytes(ci))).andExpect(status().isOk());
	...
	public void deleteReservation() throws Exception {
		...
		Guest friend = new Guest("nevena5695@gmail.com", "password12345", "name5", "name5");
		guestRepository.save(friend);
		GuestReservation gr = new GuestReservation(friend, r, true);
		gr = guestReservationRepository.save(gr);
		ReservationDetails rd = new ReservationDetails();
		rd.setGuestReservationId(gr.getId());
		rd.setReservation(r);

		this.mvc.perform(post("/api/reservation/guest/delete").contentType(MediaType.APPLICATION_JSON_VALUE)
				.content(IntegrationTestUtils.convertObjectToJsonBytes(rd))).andExpect(status().isNotFound());
	...
	public void changeOrder() throws Exception {
		...
		this.mvc.perform(post("/api/reservation/changeOrder").contentType(MediaType.APPLICATION_JSON_VALUE)
				.content(IntegrationTestUtils.convertObjectToJsonBytes(rd))).andExpect(status().isNotFound());

	}

	@Test
	public void rateRestaurant() throws Exception {
		Reservation r = new Reservation(restaurant, "20/06/2017 15:00", "20/06/2017 16:00");
		reservationReposotory.save(r);
		TableReservation tr = new TableReservation(table, r.getStartTime(), r.getEndTime());
		tr.setReservation(r);
		...
		gr = guestReservationRepository.save(gr);
		Rate rate = new Rate();
		rate.setGuest(friend);
		rate.setReservation(r);
		rate.setRateMenu(5);
		rate.setRateRestaurant(5);

		this.mvc.perform(post("/api/restaurant/rate").contentType(MediaType.APPLICATION_JSON_VALUE)
				.content(IntegrationTestUtils.convertObjectToJsonBytes(rate))).andExpect(status().isOk());

		GuestReservation g = guestReservationRepository.getGuestReservation(r.getId(), friend.getId());
		assertEquals(true, g.isRate());
	...
	public void getSystem_manager() throws Exception {
		...
		this.mvc.perform(get("/api/sysman/1").contentType(MediaType.APPLICATION_JSON_VALUE)).andExpect(status().isOk())
				.andExpect(jsonPath("$.email", is(sm.getEmail()))).andExpect(jsonPath("$.id", is(1)));
	}
	...
	public void signUp() throws Exception {
		...
		this.mvc.perform(post("/api/sysman/createSysman").contentType(MediaType.APPLICATION_JSON_VALUE)
				...
				.andExpect(jsonPath("$.email", is(sm.getEmail())))
	...
	public void getRest() throws Exception {
		...
		this.mvc.perform(get("/api/sysman/restaurant/1").contentType(MediaType.APPLICATION_JSON_VALUE))
				.andExpect(status().isOk()).andExpect(jsonPath("$.name", is(r.getName())))
	...
	public void addRest() throws Exception {
		...
		this.mvc.perform(post("/api/sysman/addRest").contentType(MediaType.APPLICATION_JSON_VALUE)
				...
				.andExpect(jsonPath("$.name", is(r.getName())));
	...
	public void getSupplier() throws Exception {
		...
		this.mvc.perform(get("/api/suppliers/2").contentType(MediaType.APPLICATION_JSON_VALUE))
				.andExpect(status().isOk()).andExpect(jsonPath("$.name", is(s.getName())))
	...
	public void Supply_not_choosed() throws Exception {
		...
		this.mvc.perform(post("/api/suppliers/supply_not_choosed").contentType(MediaType.APPLICATION_JSON_VALUE)
				.content(IntegrationTestUtils.convertObjectToJsonBytes(sm))).andExpect(status().is4xxClientError()); // jer
																														// ne
																														// postoji
																														// nijednu
																														// takva
	...
	public void getMan() throws Exception {
		// System_manager sm = new System_manager("email@gmail.com", "pass",
		// "fn", "ln");
		// sysmanRepository.save(sm);
	...
	public void getManager() throws Exception {
		...
		this.mvc.perform(get("/api/manager/1").contentType(MediaType.APPLICATION_JSON_VALUE)).andExpect(status().isOk())
				.andExpect(jsonPath("$.email", is(sm.getEmail()))).andExpect(jsonPath("$.id", is(1)));
	}
	...
	public void getRestManager() throws Exception {
		...
		this.mvc.perform(post("/api/manager/getRest").contentType(MediaType.APPLICATION_JSON_VALUE)
				.content(IntegrationTestUtils.convertObjectToJsonBytes(sm))).andExpect(status().isOk())
				.andExpect(jsonPath("$.id", is((int) (long) find)));
	}
	...
	public void regions() throws Exception {
		...
		this.mvc.perform(post("/api/manager/regions").contentType(MediaType.APPLICATION_JSON_VALUE)
				.content(IntegrationTestUtils.convertObjectToJsonBytes(r))).andExpect(status().isOk());
	}
	...
	public void supply() throws Exception {
		...
		this.mvc.perform(post("/api/manager/supply").contentType(MediaType.APPLICATION_JSON_VALUE)
				.content(IntegrationTestUtils.convertObjectToJsonBytes(r))).andExpect(status().isOk());

		this.mvc.perform(post("/api/manager/supply_hist").contentType(MediaType.APPLICATION_JSON_VALUE)
				.content(IntegrationTestUtils.convertObjectToJsonBytes(r))).andExpect(status().isOk());
	}
	...
	public void getSupplyChoosed() throws Exception {
		...
		if (supply.size() == 0)
			this.mvc.perform(post("/api/manager/supply_choosed").contentType(MediaType.APPLICATION_JSON_VALUE)
					.content(IntegrationTestUtils.convertObjectToJsonBytes(s))).andExpect(status().is4xxClientError());
		else
			this.mvc.perform(post("/api/manager/supply_choosed").contentType(MediaType.APPLICATION_JSON_VALUE)
